...
#                                                                      #
#                                                                      #
# PURPOSE: A script for 2 Sockets 2 NUMA nodes MLC test                #
#                                                                      #
# VERSION: 1.0.0                                                       #
#                                                                      #
# Author: Peter Xu  
...
def check_nps():
    if int(get_numa_node_count()/get_socket_count())!= 1:
        print("Please set NPS=1 in the UEFI")
        return False
    else:
        return True
...
def check_for_requirements():
    ...
    if not check_nps():
       sys.exit("NPS=1 is required, please change the setting in the UEFI")
...
def run_mlc():
    ...
    if check_mlc_file():
        ...
        if os.path.exists("mlc_test_nps1.log"):
            target = str(os.getcwd()) + '/' + 'mlc_test_nps1' + '_' + str(Current_Time) + '.log'
            os.rename("mlc_test_nps1.log",target)
        print(run_shell("./mlc | tee mlc_test_nps1.log"))
...
def data_treatment():
    lat = []
    count = 0
    with open("mlc_test_nps1.log","r") as f:
    ...
    with open("mlc_test_nps1.log","r") as x:
        lines = x.readlines()
    ...
    if os.path.exists("mlc_spv_data_nps1.log"):
        Current_Time=datetime.today().strftime('%Y-%m-%d-%H:%M:%S')
        target = str(os.getcwd()) + '/' + 'mlc_spv_data_nps1' + '_' + str(Current_Time) + '.log'
        os.rename("mlc_spv_data_nps1.log",target) 

    #redirect 'print' output to a file
    sys.stdout=open("mlc_spv_data_nps1.log","w")
    print(local_node_latency)
    print(remote_node_latency)
    print(local_node_bandwith)
    print(remote_node_bandwith)
    ...
    plt.xlim(0,4)
    plt.ylim(0,400)
    plt.title('MLC NPS1 Latencies(in ns)')
    plt.legend()
    plt.show(block=False)
    plt.pause(3)
    if os.path.exists("mlc_nps1.jpg"):
        Current_Time=datetime.today().strftime('%Y-%m-%d-%H:%M:%S')
        target = str(os.getcwd()) + '/' + 'mlc_nps1' + '_' + str(Current_Time) + '.jpg'
        os.rename("mlc_nps1.jpg",target)
    plt.savefig('mlc_nps1.jpg')
    plt.close()
    ...
    remote_node_1_bw = list(np.array(remote_node_bw).flatten()/1000)
    local_node_1_bw = local_node_bw/1000
    plt.plot(x1,local_node_1_bw,marker='*',color='green',label=u'local node')
    plt.plot(x3,remote_node_1_bw,marker='s',color='red',label=u'remote node')
    plt.ylabel(u'Bandwidth(GB/s)')
    plt.xlim(0,4)
    plt.ylim(0,400)
    plt.title('MLC NPS1 Bandwidth(in GB/s)')
    plt.legend()
    plt.show(block=False)
    plt.pause(3)
    if os.path.exists("mlc_bw_nps1.jpg"):
        Current_Time=datetime.today().strftime('%Y-%m-%d-%H:%M:%S')
        target = str(os.getcwd()) + '/' + 'mlc_bw_nps1' + '_' + str(Current_Time) + '.jpg'
        os.rename("mlc_bw_nps1.jpg",target)
    plt.savefig('mlc_bw_nps1.jpg')
    plt.close()
