...
#                                                                      #
#                                                                      #
# PURPOSE: A script for 2 Sockets 4 NUMA nodes MLC test                #
#                                                                      #
# VERSION: 1.0.0                                                       #
#                                                                      #
# Author: Peter Xu  
...
def check_nps():
    if int(get_numa_node_count()/get_socket_count())!= 2:
        print("Please set NPS=2 in the UEFI")
        return False
    else:
        return True
...
def run_mlc():
    ...
    if check_mlc_file():
        ...
        if os.path.exists("mlc_test_nps2.log"):
            target = str(os.getcwd()) + '/' + 'mlc_test_nps2' + '_' + str(Current_Time) + '.log'
            os.rename("mlc_test_nps2.log",target)
        print(run_shell("./mlc | tee mlc_test_nps2.log"))
...
def data_treatment():
    lat = []
    count = 0
    with open("mlc_test_nps2.log","r") as f:
    ...
    with open("mlc_test_nps2.log","r") as x:
        lines = x.readlines()
    ...
    if os.path.exists("mlc_spv_data_nps2.log"):
        Current_Time=datetime.today().strftime('%Y-%m-%d-%H:%M:%S')
        target = str(os.getcwd()) + '/' + 'mlc_spv_data_nps2' + '_' + str(Current_Time) + '.log'
        os.rename("mlc_spv_data_nps2.log",target) 

    #redirect 'print' output to a file
    sys.stdout=open("mlc_spv_data_nps2.log","w")
    print(local_node_latency)
    print(near_local_node_latency)
    print(remote_node_latency)
    print(local_node_bandwith)
    ...
    plt.pause(3)
    if os.path.exists("mlc_nps2.jpg"):
        Current_Time=datetime.today().strftime('%Y-%m-%d-%H:%M:%S')
        target = str(os.getcwd()) + '/' + 'mlc_nps2' + '_' + str(Current_Time) + '.jpg'
        os.rename("mlc_nps2.jpg",target)
    plt.savefig('mlc_nps2.jpg')
    plt.close()
    ...
    plt.show(block=False)
    plt.pause(3)
    if os.path.exists("mlc_bw_nps2.jpg"):
        Current_Time=datetime.today().strftime('%Y-%m-%d-%H:%M:%S')
        target = str(os.getcwd()) + '/' + 'mlc_bw_nps2' + '_' + str(Current_Time) + '.jpg'
        os.rename("mlc_bw_nps2.jpg",target)
    plt.savefig('mlc_bw_nps2.jpg')
    plt.close()
